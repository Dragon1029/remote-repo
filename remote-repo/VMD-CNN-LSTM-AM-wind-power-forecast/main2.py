...
wind_power = pd.read_csv('./data2/handle_Turbine_Data .csv')
print(wind_power.info)
wind_power.dropna(inplace=True)
print(wind_power.info)
# training_set = wind_power.iloc[0: - 6000, 0:1].values  # 前(2426-300=2126)天的开盘价作为训练集,表格从0开始计数，2:3 是提取[2:3)列，前闭后开,故提取出C列开盘价
# test_set = wind_power.iloc[32284 - 6000:, 0:1].values  # 后300天的开盘价作为测试集
training_set = wind_power.iloc[0: - 6000, 0:1].values  # 前(2426-300=2126)天的开盘价作为训练集,表格从0开始计数，2:3 是提取[2:3)列，前闭后开,故提取出C列开盘价
test_set = wind_power.iloc[35040 - 6000:, 0:1].values  # 后300天的开盘价作为测试集
# 归一化
